...
import { G_DOLLAR, CUSD, NFPM_ADDRESS } from '../modules/config';
...
export class StrategyWorker extends WorkerHost {
  private readonly MIN_DEPLOY = parseUnits('100000', G_DOLLAR.decimals);
  private readonly DEPLOY_RATIO = 9500n; // 95% of idle
  ...
  private async deployAndLp(): Promise<void> {
    ...
    // 2. Tick range
    const baseTick = nearestUsableTick(pool.tickCurrent, pool.tickSpacing);
    const tickSpacing = pool.tickSpacing;
    const tickLower = baseTick - tickSpacing * this.TICK_RANGE;
    const tickUpper = baseTick + tickSpacing * this.TICK_RANGE;
    this.logger.log(`Tick range: [${tickLower}, ${tickUpper}]`);

    // 3. Check for existing in-range position
    const existingTokenId =
      await this.positionRegistry.findActivePositionInTickRange(
        tickLower,
        tickUpper,
      );
    const isInRange =
    ...
    if (available >= this.MIN_DEPLOY) {
      ...
      this.logger.log(
        `Deploy: ${formatUnits(deployAmount, G_DOLLAR.decimals)} G$ (swap half: ${formatUnits(halfAmount, G_DOLLAR.decimals)})`,
      );
      ...
      this.logger.log(
        `Swap quote: ${halfFormatted} G$ → ${formatUnits(cUsdQuote, CUSD.decimals)} cUSD`,
      );

      const [deployNonce] = await this.nonceManager.allocate(1);
      const deployTx = await this.treasury.deployToStrategy(account, deployAmount, deployNonce);
      const deployReceipt = await this.client.waitForTransactionReceipt({
        hash: deployTx,
        timeout: 120_000,
      });
      if (deployReceipt.status === 'reverted') {
        this.logger.error('DeployToStrategy reverted — aborting');
        throw new Error('DeployToStrategy transaction reverted');
      }
      this.logger.log('Deploy confirmed');

      await this.swap.executeSwap(G_DOLLAR, halfFormatted, cUsdQuote);
      this.logger.log('Swap confirmed');

      pool = await this.poolState.fetchPoolState();
      this.logger.log(`Post-swap pool tick: ${pool.tickCurrent}`);
    } else {
      this.logger.log(
        `Skipping deploy: ${formatUnits(available, G_DOLLAR.decimals)} G$ < ${formatUnits(this.MIN_DEPLOY, G_DOLLAR.decimals)} G$`,
      );
    }
    ...
    const [actualG$, actualCUsd] = await Promise.all([
      this.client.readContract({
        address: G_DOLLAR.address as `0x${string}`,
        abi: balanceOfAbi,
        functionName: 'balanceOf',
        args: [accountAddr],
      }) as Promise<bigint>,
      this.client.readContract({
        address: CUSD.address as `0x${string}`,
        abi: balanceOfAbi,
        functionName: 'balanceOf',
        args: [accountAddr],
      }) as Promise<bigint>,
    ...
    this.logger.log(
      `Account balances: ${formatUnits(actualG$, G_DOLLAR.decimals)} G$, ${formatUnits(actualCUsd, CUSD.decimals)} cUSD`,
    );
    ...
    const isGDollarToken0 =
      pool.token0.address.toLowerCase() === G_DOLLAR.address.toLowerCase();
    const amount0 = isGDollarToken0 ? actualG$ : actualCUsd;
    const amount1 = isGDollarToken0 ? actualCUsd : actualG$;

    // 6. Mint or increase
    if (existingTokenId !== null && isInRange) {
      this.logger.log(`Increasing liquidity tokenId=${existingTokenId}`);
      const increaseTx = await this.position.increaseLiquidity(
        pool,
        existingTokenId,
        amount0,
        amount1,
      ...
      const mintTx = await this.position.mintPosition(
        pool,
        amount0,
        amount1,
        tickLower,
      ...
      if (receipt.status === 'reverted') {
        this.logger.error(
          `Mint reverted! Logs: ${JSON.stringify(receipt.logs)}`,
        );
      ...
      const mintLog = receipt.logs.find(
        (l) =>
          l.address.toLowerCase() === NFPM_ADDRESS.toLowerCase() &&
          l.topics[0] === TRANSFER_EVENT &&
          l.topics[1] === ZERO_ADDRESS_TOPIC,
      );
      if (!mintLog?.topics[3])
        throw new Error('Mint Transfer event not found in logs');
      ...
      await this.positionRegistry.registerPosition(
        tokenId,
        tickLower,
        tickUpper,
      );
